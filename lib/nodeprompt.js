...
try {
	configUser = require( ( process.env.HOME || process.env.HOMEPATH || process.env.USERPROFILE ) + '/.nodeprompt/config.user.js' );
} catch ( e ) {}
...
for ( var c in NODEPROMPT.configDefault ) {
	NODEPROMPT.config[ c ] = NODEPROMPT.configUser[ c ] || NODEPROMPT.configDefault[ c ];
}
