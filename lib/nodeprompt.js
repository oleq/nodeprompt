...
class Nodeprompt {
	...
	_getModel() {
		...
		if ( model.isGit ) {
			...
			model.isDetached = !!( !head.match( detachedRegex ) || isBisecting );
	...
	_getStatus( model ) {
		...
		if ( !model.isDetached && !model.isInit ) {
			...
			model.branch = branchLine.match( branchRegex )[ 0 ];
		}
		...
		model.ahead = ahead;
		model.behind = behind;
		model.hasDiverged = !!( ahead && behind );
	}
